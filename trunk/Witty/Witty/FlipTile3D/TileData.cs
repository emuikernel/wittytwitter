...
namespace Witty
{
    internal class TileData
    {
            ...
            Size size, Point center, Material backMaterial, Rect backTextureCoordinates)
        {
            ...
            _quad.Children.Add(CreateTile(diffuseMaterialBrushPair, backMaterial, _borderMaterial, new Size3D(size.Width, size.Height, .01),
            ...
            Size3D size, Rect backMaterialCoordiantes)
        {
            ...
            RotateTransform3D backRotate = new RotateTransform3D(new AxisAngleRotation3D(new Vector3D(0, 1, 0), 180), new Point3D());
            RotateTransform3D backFlip = new RotateTransform3D(new AxisAngleRotation3D(new Vector3D(0, 0, 1), 180), new Point3D());
        ...
        private static GeometryModel3D GetQuad(Point3D bottomLeft, Point3D bottomRight, Point3D topRight, Point3D topLeft,
        ...
        private static Vector limitVector(Vector input, double max)
        {
            Debug.Assert(max > 0);
            Debug.Assert(!double.IsPositiveInfinity(max));
            Debug.Assert(!double.IsNaN(input.Length));
        ...
        private Vector DesiredLocationVector { get { return new Vector(_locationDesired.X, _locationDesired.Y); } }
        private Vector3D CurrentLocationVector { get { return new Vector3D(_locationCurrent.X, _locationCurrent.Y, _locationCurrent.Z); } }
